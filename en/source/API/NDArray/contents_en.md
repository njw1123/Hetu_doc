...
- `slice(begin_pos, output_shape) -> hetu.NDArray`: Slices the NDArray according to the specified beginning position and output shape.
...
- `transpose(perms)->hetu.NDArray`: Transposes the NDArray according to the specified permutation of axes.
  - `perms (List[int])`: A list of integers specifying the new order of axes.
- `view(shape) -> hetu.NDarray`: Creates a view of the NDArray with the specified shape. This does not copy the data, but rather creates a new view of the original data with the given shape.
