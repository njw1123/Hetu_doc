...
run(loss:hetu.Tensor, fetch:List[Tensor], feed_dict:FeedDict=None, num_micro_batches:int=1, cur_strategy_id:int=0, run_level:int=0, save_checkpoint:bool=false, grad_scale:double=1, num_tokens:int=0) -> List[hetu.NDArray]
...
optimize_strategy_id:int=0, run_level:int=0, save_checkpoint:bool=false, grad_scale:double=1, num_tokens:int=0) -> List[hetu.NDArray]
```
...
- `cur_strategy_id (int)`: The ID of the currently active strategy. Hetu supports dynamic switching between different strategies during execution.
- `compute_strategy_id (int)`: The strategy used for computation. Hetu supports different strategies for computation and optimization.
- `optimize_strategy_id (int)`:The strategy used for optimization. Like compute_strategy_id, Hetu supports separate strategies for computation and optimization.
...
- `save_checkpoint (bool)`: Whether to save the model checkpoint during execution.
- `grad_scale (float)`: The factor by which gradients are scaled during training
- `num_tokens (int)`: The number of global batch tokens in the current run, which is used for token-level loss computation and gradient scaling.
